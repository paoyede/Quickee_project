  ...
  consumeMessages(): void {
    ...
    this.channel.consume(queueName, async (msg: amqp.ConsumeMessage | null) => {
      if (msg !== null) {
        ...
        let mail = "";
        const getDictionary = new Map(payload.Payload);
        const code = getDictionary.get("Code").toString();
        if (payload.EmailTemplate === "studentreg") {
          mail = studentRegContent(payload.Name, code);
        } else if (payload.EmailTemplate === "kitchenreg") {
          mail = kitchenRegContent(payload.Name, code);
        } else if (payload.EmailTemplate === "forgotpassword") {
          mail = forgotPassContent(payload.Name, code);
        }
